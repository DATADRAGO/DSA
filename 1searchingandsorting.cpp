...
void bubblesort(student s[50], int n); //function declaration
...
cout<<"Enter sgpa\n";
 ...
 displayinfo(s, n); //function call
 bubblesort(s,n); //function call
 quicksort(s, 0, n-1); //function call
 displayinfoReverse(s, n); //function call
 insertion(s,n); //function call
...
s[j].sgpa=temp;
 ...
 strcpy(s[j].name,temp2);
quicksort(s,first, j-1); //recursive function call left part
quicksort(s, j+1, last); // recursive call for right side
}
}
void linearsearch(student s[50], int n, float key)
{
